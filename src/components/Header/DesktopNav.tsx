        ...
        <div className="hidden md:flex items-center gap-6">
            <nav className="flex items-center gap-6">
                {links.map((link) => (
                    ...
                    >
                        {link.name}
                        <span className={`absolute bottom-0 left-0 h-0.5 bg-linear-to-r from-blue-400 to-purple-500 transition-all duration-300 ${pathname === link.href ? "w-full" : "w-0 group-hover:w-full"
                            }`} />
                    </Link>
                ))}
            </nav>
